...
import { isLiveKitConfigured, roomService } from "@/lib/livekit";
 ...
 *   • live    — asked of the video server, right now
 *
...
export async function getTeachingUsage() {
  try {
    ...
    const [rows, lessonsHeld, admins] = await Promise.all([
      ...
      User.find({ role: { $ne: "student" } }, { _id: 1 }).lean(),
    ]);
...
/**
 * Who is connected to a classroom at this very moment. Asked of the video
 * server directly — no database, no history. Rooms disappear from the answer a
 * few minutes after the last person leaves, which is why an ended class stops
 * showing up on its own.
 */
export async function getLiveClassrooms() {
  if (!isLiveKitConfigured()) return { ok: false, configured: false, people: 0, rooms: [] };
  try {
    const all = await roomService().listRooms();
    const busy = all.filter((r) => (r.numParticipants || 0) > 0);

    const lessonIds = busy
      .map((r) => String(r.name || "").replace(/^ddd-lesson-/, ""))
      .filter((id) => mongoose.isValidObjectId(id));

    let byLesson = new Map();
    if (lessonIds.length) {
      await connectDB();
      const lessons = await Lesson.find({ _id: { $in: lessonIds } }, { title: 1, course: 1 })
        .populate("course", "title level")
        .lean();
      byLesson = new Map(lessons.map((l) => [String(l._id), l]));
    }

    const rooms = busy
      .map((r) => {
        const id = String(r.name || "").replace(/^ddd-lesson-/, "");
        const lesson = byLesson.get(id);
        const startedAt = r.creationTime ? new Date(Number(r.creationTime) * 1000).toISOString() : null;
        return {
          id,
          people: r.numParticipants || 0,
          publishing: r.numPublishers || 0,
          title: lesson?.course?.title || lesson?.title || null,
          level: lesson?.course?.level || "",
          lesson: lesson?.title || "",
          startedAt,
          minutes: startedAt ? Math.max(0, Math.round((Date.now() - new Date(startedAt).getTime()) / 60000)) : null,
        };
      })
      .sort((a, b) => b.people - a.people);

    return { ok: true, configured: true, people: rooms.reduce((sum, r) => sum + r.people, 0), rooms };
  } catch (err) {
    return { ok: false, configured: true, people: 0, rooms: [], error: err?.message || "could not reach the video server" };
  }
}
...
export async function getUsageReport() {
  const [storage, traffic, teaching, live] = await Promise.all([
    getStorageUsage(),
    getTrafficUsage(),
    getTeachingUsage(),
    getLiveClassrooms(),
  ...
  return { storage, traffic, teaching, live, bytesPerPersonMinute };
}
